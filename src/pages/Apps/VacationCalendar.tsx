...
const VacationCalendar: React.FC = () => {
  ...
  const getColorForSucursal = (sucursal: string): string => {
    const sucursalUpper = sucursal?.toUpperCase() || '';
    switch (sucursalUpper) {
      case 'MDP': return '#DBEAFE';      // Celeste pastel (bg-blue-100)
      case 'PICO': return '#D1FAE5';     // Verde pastel (bg-green-100)
      case 'ROS':
      case 'ROSARIO': return '#F3F4F6';  // Gris pastel (bg-gray-100)
      case 'DIMES': return '#93C5FD';    // Azul oscuro pastel (bg-blue-300)
      default: return '#E5E7EB';         // Gris claro por defecto
    }
  };
  ...
  const events: EventInput[] = filteredEvents.map(evento => {
    const colaborador = colaboradores.find(c => c.colaboradorID === evento.colaboradorID);
    let title, start, end;
    const backgroundColor = getColorForSucursal(colaborador?.sucursal || '');
    ...
    return {
      ...
      textColor: '#1F2937', // Gris oscuro para mejor contraste con colores pasteles
    ...
    <div className="container mx-auto p-6 bg-white min-h-screen">
      ...
      <div className="mb-4 flex justify-center space-x-6">
        <div className="flex items-center">
          <div className="w-4 h-4 rounded mr-2" style={{ backgroundColor: '#DBEAFE' }}></div>
          <span>MDP</span>
        </div>
        <div className="flex items-center">
          <div className="w-4 h-4 rounded mr-2" style={{ backgroundColor: '#D1FAE5' }}></div>
          <span>PICO</span>
        </div>
        <div className="flex items-center">
